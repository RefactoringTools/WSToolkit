...
parseDocument(Xml, S) when is_list(Xml) ->
  ws_erlsom_sax_list:parse(Xml, S);

parseDocument(Xml, S) when is_binary(Xml) ->
  case S#erlsom_sax_state.encoding of
    undefined ->
      {Encoding, Xml2, CState2} = 
        ws_erlsom_lib:detectEncoding(Xml, S#erlsom_sax_state.continuation_fun,
...
parseDocumentBinary(Encoding, Xml, State) ->
  case Encoding of 
    'utf8' -> 
      ws_erlsom_sax_utf8:parse(Xml, State);
    'utf16be' -> 
      ws_erlsom_sax_utf16be:parse(Xml, State);
    'utf16le' -> 
      ws_erlsom_sax_utf161e:parse(Xml, State);
    'latin-1' ->
      ws_erlsom_sax_latin1:parse(Xml, State);
    'iso_8859_1' ->
      ws_erlsom_sax_latin1:parse(Xml, State);
    'iso_8859_15' ->
      ws_erlsom_sax_latin9:parse(Xml, State);
    'list' ->
      ws_erlsom_sax_list:parse(Xml, State);
    _ ->
