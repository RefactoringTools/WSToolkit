...
compile_internal(Xsd, Options, Parsed) ->
  ...
  IncludeFun = case lists:keysearch('include_fun', 1, Options) of
                 {value, {_, If}} -> If;
                 _ -> fun ws_erlsom_lib:findFile/4
               end,
  ...
  ParsingResult = case Parsed of
                    false -> ws_erlsom_parseXsd:parseXsd(Xsd, Ns);
                    _ -> Xsd
                  end,
...
compile_parsed_xsd(ParsedXsd, Prefix, Namespaces) ->
  compile_parsed_xsd(ParsedXsd, Prefix, Namespaces, fun ws_erlsom_lib:findFile/4, ["."], []).
...
compile_parsed_xsd(ParsedXsd, Prefix, Namespaces, IncludeFun, IncludeDirs, IncludeFiles) ->
  ...
  case catch transform(ParsedXsd, Acc) of
    ...
    IntermediateResult -> 
      case catch ws_erlsom_pass2:secondPass(IntermediateResult#p1acc.tps,
  ...
  case lists:member(Ns, Imported) of
    ...
    _ ->
      ...
      Prefix = case Prefix1 of
                 undefined ->
                   ws_erlsom_lib:findPrefix2(Ns, Namespaces);
                 _ -> Prefix1
               end,
      ParsedGrammar = case Xsd of
                        #schemaType{} -> 
                          Xsd;
                        _ -> 
                          ws_erlsom_parseXsd:parseXsd(Xsd, Acc#p1acc.nss)
                      end,
...
processImports([Impt = #includeType{} | Tail], Acc = #p1acc{includeDirs = Dirs,
  ...
  ParsedGrammar = ws_erlsom_parseXsd:parseXsd(Xsd, Acc#p1acc.nss),
...
replaceElements(ImportedTypes, [Original = #typeInfo{typeName= Name, typeType = TypeT} | Tail], 
  ...
  case lists:keysearch(Name, #typeInfo.typeName, TypesToRedefine) of
    ...
    {value, Redefined = #typeInfo{base = Base, anyAttr = AnyAttr, elements = Elemts, attributes = Attrs}} -> 
      RedefinedType = case ws_erlsom_lib:searchBase(ws_erlsom_lib:makeTypeRef(Base, Namespaces), ImportedTypes) of
                        {value, #typeInfo{elements = BaseEls, attributes = BaseAttrs, 
                          ...
                          Redefined#typeInfo{elements = ws_erlsom_lib:emptyListIfUndefined(BaseEls) ++
                                                        ws_erlsom_lib:emptyListIfUndefined(Elemts),
                                             attributes = BaseAttrs ++ Attrs,  
                                             anyAttr = NewAnyAttr,
                                             extends = Base2};
                        _Else ->
                          throw({error, "Base type not found: " ++ ws_erlsom_lib:makeTypeRef(Base, Namespaces)})
                      end,
      replaceElements(ImportedTypes, Tail, TypesToRedefine, [RedefinedType | Acc], Namespaces);
    _ ->
...
transformTypes([#globalElementType{name=Name, type=undefined, simpleOrComplex=SorC,
   ...
   SubstitutionGroup = case SubstGroup of 
                         undefined -> undefined;
                         _ -> ws_erlsom_lib:makeElementRef(SubstGroup, Nss)
                       end,
   case SorC of 
      #localSimpleTypeType{} ->
        ThisType = #typeInfo{typeName= ws_erlsom_lib:makeElementName(Name, Prefix),global = true,
                        typeRef = "##string",
      ...
      #localComplexTypeType{} ->
        ...
        Type2 = Type#typeInfo{typeName= ws_erlsom_lib:makeElementName(Name, Prefix), global = true,
                              substitutionGroup = SubstitutionGroup,
...
transformTypes([#globalElementType{name=Name, type=Type, simpleOrComplex=undefined,
   ...
   ThisType = #typeInfo{typeName= ws_erlsom_lib:makeElementName(Name, Prefix),global = true,
                        typeRef = ws_erlsom_lib:makeTypeRef(Type, Nss),
                        substitutionGroup = case SubstGroup of
                                              undefined -> undefined;
                                              _ -> ws_erlsom_lib:makeElementRef(SubstGroup, Nss)
                                            end,
...
transformTypes([#globalComplexTypeType{name=Name, model=Model, attributes=Attributes, anyAttribute=AnyAttr, mixed = Mixed} |
   ...
   Type2 = Type#typeInfo{typeName= ws_erlsom_lib:makeTypeName(Name, Prefix), global = true,
...
transformTypes([#globalSimpleTypeType{name=Name}| Tail],
   ...
   Type = #typeInfo{typeName = ws_erlsom_lib:makeTypeName(Name, Prefix), typeRef = "##string", global = false,
                            typeType = simpleType},
...
makeNewType(Name, Model, Acc = #p1acc{nsp = Prefix}) ->
   Path = Name ++ "/",
   {Type, Acc2} = translateComplexTypeModel(Model, Acc#p1acc{path = Path}),
   {Type#typeInfo{typeName= ws_erlsom_lib:makeTypeName(Name, Prefix), global = false, typeType = type}, Acc2}.
...
transformGroupDef(#groupDefType{name=Name, model=Model}, 
   ...
   Path = Name ++ "/",
   {Type, Acc2} = translateComplexTypeModel(Model, Acc#p1acc{path = Path}),
   {Type#typeInfo{typeName= ws_erlsom_lib:makeGroupName(Name, Prefix), global = false, typeType = type}, Acc2}.
...
translateAttribute(#localAttributeType{name=Name, type=Type, use=Use, ref=Ref,
  ...
  #attrib{name = ws_erlsom_lib:makeTag(Name, Prefix, Form2), type=Type, optional=Use, ref=Ref};
...
translateComplexTypeModel(#anyType{minOccurs=Min, maxOccurs = Max, namespace = Ns, processContents = Pc}, 
                          Acc = #p1acc{tns = Tns}) ->
   AnyInfo = #anyInfo{prCont = case Pc of undefined -> "strict"; _ -> Pc
                               end,
...
translateComplexTypeModel(#groupRefType{ref=Ref, minOccurs=Min, maxOccurs = Max}, Acc = #p1acc{nss = Nss}) ->
  Elements = [#elementInfo{alternatives=[#alternative{tag="##TODO", type=ws_erlsom_lib:makeGroupRef(Ref, Nss), real=false}],
                           min= minMax(Min), max= minMax(Max)}], 
...
translateElement(#groupRefType{ref=Ref, minOccurs=Min, maxOccurs = Max}, Acc = #p1acc{nss = Nss}) ->
  Element = #elementInfo{alternatives=[#alternative{tag="##TODO", type=ws_erlsom_lib:makeGroupRef(Ref, Nss), real=false}],
                           min=minMax(Min), max=minMax(Max)}, 
  {Element, Acc}.
...
translateAlternative(#localElementType{type=undefined, ref=Ref, simpleOrComplex=undefined,
  ...
  Ref /= undefined ->
  {#alternative{tag= ws_erlsom_lib:makeTagFromRef(Ref, Nss), type=ws_erlsom_lib:makeElementRef(Ref, Nss), real = true,
...
translateAlternative(#localElementType{name=Name, type=undefined, ref=undefined, simpleOrComplex=SorC,
   ...
   case SorC of 
      ...
      #localComplexTypeType{} ->
         ...
         ResultSoFar = Acc2#p1acc.tps,
         TypeRef = ws_erlsom_lib:makeTypeName(Name, Form2, Path, Prefix),
   ...
   {#alternative{tag=ws_erlsom_lib:makeTag(Name, Prefix, Efd), type=TypeRef, real = true, min=minMax(Min),
                 max=minMax(Max)}, Acc3};
...
translateAlternative(#localElementType{name=Name, type=Type, ref=undefined, simpleOrComplex=undefined,
   ...
   Form2 = case Form of undefined -> Efd; _ -> Form end,
   {#alternative{tag=ws_erlsom_lib:makeTag(Name, Prefix, Form2), type=ws_erlsom_lib:makeTypeRef(Type, Nss),
...
translateAlternative(#groupRefType{ref=Ref, minOccurs=Min, maxOccurs=Max}, Acc = #p1acc{nss = Nss}) ->
  {#alternative{tag="##TODO", type=ws_erlsom_lib:makeGroupRef(Ref, Nss), real=false, min=minMax(Min), max=minMax(Max)}, Acc};
...
translateAlternative(Sequence = #sequenceType{minOccurs=Min, maxOccurs=Max}, 
  ...
  Count2 = Count + 1,
  Name = "SEQ" ++ integer_to_list(Count2),
  TypeName = ws_erlsom_lib:makeTypeName(Name, Efd, Path, Prefix),
...
translateQuasiAlternative(#localElementType{type=undefined, ref=Ref, simpleOrComplex=undefined}, 
  ...
  {#alternative{tag= ws_erlsom_lib:makeTagFromRef(Ref, Nss), type=ws_erlsom_lib:makeElementRef(Ref, Nss), real = true}, Acc};
%% A child element exists - russian doll design
translateQuasiAlternative(#localElementType{name=Name, type=undefined, ref=undefined, simpleOrComplex=SorC,
   ...
   {#alternative{tag=ws_erlsom_lib:makeTag(Name, Prefix, Efd), type=TypeRef, real = true}, Acc3#p1acc{path=Path}};
translateQuasiAlternative(#localElementType{name=Name, type=Type, ref=undefined, simpleOrComplex=undefined, form = Form}, 
                          Acc = #p1acc{efd = Efd, nsp = Prefix, nss = Nss}) ->
   Form2 = case Form of undefined -> Efd; _ -> Form end,
   {#alternative{tag=ws_erlsom_lib:makeTag(Name, Prefix, Form2), type=ws_erlsom_lib:makeTypeRef(Type, Nss), real = true}, Acc}.
%% %% -record(groupRefType, {ref, minOccurs, maxOccurs}).
