...
add(Xsd, Options, Model1) ->
  {ok, Model2} = ws_erlsom:compile_xsd(Xsd, Options),
  add_model(Model1, Model2).

add_xsd_model(Model1) ->
  add_model(Model1, ws_erlsom_parseXsd:xsdModel()).
...
add_model(Model1 = #model{tps = Tps, nss = Nss, tns = Tns, th = Th}, 
  ...
  UpdatedTypes = ws_erlsom_pass2:pass5(CombinedTypes, Info),
