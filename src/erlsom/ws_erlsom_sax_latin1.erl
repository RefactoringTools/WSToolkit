...
-define(CF3(A, B, C), ws_erlsom_sax_lib:continueFun(A, B, C)).
-define(CF4(A, B, C, D), ws_erlsom_sax_lib:continueFun(A, B, C, D)).
-define(CF4_2(A, B, C, D), ws_erlsom_sax_lib:continueFun2(A, B, C, D)).
-define(CF5(A, B, C, D, E), ws_erlsom_sax_lib:continueFun(A, B, C, D, E)).
-define(CF6(A, B, C, D, E, F), ws_erlsom_sax_lib:continueFun(A, B, C, D, E, F)).
-define(CF6_2(A, B, C, D, E, F), ws_erlsom_sax_lib:continueFun2(A, B, C, D, E, F)).
...
parseEntityName(Tail, State = #erlsom_sax_state{max_entity_size = MaxSize,
  ...
  ValueAsList = case OutputEncoding of
                  'utf8' -> ws_erlsom_ucs:decode_utf8(Value);
                  _ -> Value
                end,
  if
...
parseTextNoIgnoreBinary(Head, ?STR1_T($&, Tail), State) ->
  ...
  Head2Binary = list_to_binary(ws_erlsom_ucs:to_utf8(lists:reverse(Head2))),
...
parseTextNoIgnoreBinary(Head, Tail, State) ->
  {Char, Tail2, State2} = decodeChar(Tail, State),
  EncodedChar = ws_erlsom_ucs:char_to_utf8(Char),
    ...
    definition ->
      ...
      case lists:member(NewRelation, Relations) of
        ...
        false ->
          ...
          case ws_erlsom_sax_lib:findCycle(Reference, CurrentEntity, Relations2, MaxDepth) of
            cycle -> 
...
-ifdef(UTF8).
encode(List) ->
  list_to_binary(ws_erlsom_ucs:to_utf8(List)).
-endif.
...
decodeIfRequired(URI) when is_binary(URI) ->
  {Value, _} = ws_erlsom_ucs:from_utf8(URI),
  Value.
...
-ifdef(UTF8).
decode(Bin) ->
  {Value, _} = ws_erlsom_ucs:from_utf8(Bin),
  Value.
-endif.
...
-ifdef(U16B).
decode(Bin) ->
  {Value, _} = ws_erlsom_ucs:from_utf16be(Bin),
  Value.
-endif.

-ifdef(U16L).
decode(Bin) ->
  {Value, _} = ws_erlsom_ucs:from_utf16le(Bin),
  Value.
-endif.
