...
compile_xsd(Xsd, Options) ->
  case catch ws_erlsom_compile:compile(Xsd, Options) of
...
scan2(Xml, State, Options) ->
  case catch ws_erlsom_sax:parseDocument(Xml, State,
                                         fun ws_erlsom_parse:xml2StructCallback/2,
                                         Options) of
    {error, Message} -> {error, Message};
    {'EXIT', Message} -> throw({'EXIT', Message});
...
parse_file(File, Model) ->
  case file:read_file(File) of
    {ok, Bin} ->
      parse(ws_erlsom_lib:toUnicode(Bin), Model);
    Error ->
      Error
  end.
...
simple_form(Xml, Options) ->
  ws_erlsom_simple_form:scan(Xml, Options).
...
%%----------------------------------------------------------------------
write(Struct, Model) ->
  ws_erlsom_write:write(Struct, Model).
...
parse_sax(Xml, State, EventFun, Options) ->
  ws_erlsom_sax:parseDocument(Xml, State, EventFun, Options).
...
write_hrl(Model, Output) ->
  Hdr = ws_erlsom_writeHrl:writeHrl(Model),
...
write_xsd_hrl_file(Xsd, Output, Options) ->
  case file:read_file(Xsd) of
    {ok, Bin} ->
      Hdr = ws_erlsom_writeHrl:writeXsdHrlFile(ws_erlsom_lib:toUnicode(Bin), Options),
      file:write_file(Output, Hdr);
    Error ->
      Error
  end.
...
write_hrl_file(Xsd, Prefix, Namespaces, Output) ->
  case file:read_file(Xsd) of
    {ok, Bin} ->
      Hdr = ws_erlsom_writeHrl:writeHrlFile(ws_erlsom_lib:toUnicode(Bin), Prefix, Namespaces),
      file:write_file(Output, Hdr);
    Error ->
      Error
  end.
...
add_xsd_file(XsdFile, Options, Model) ->
  case file:read_file(XsdFile) of
    {ok, Bin} ->
      {ok, ws_erlsom_add:add(ws_erlsom_lib:toUnicode(Bin), Options, Model)};
    Error ->
      Error
  end.
...
add_xsd_model(Model) ->
  ws_erlsom_add:add_xsd_model(Model).

%% add Model2 to Model1
add_model(Model1, Model2) ->
  ws_erlsom_add:add_model(Model1, Model2).
